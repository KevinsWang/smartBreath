...
Java_cn_edu_nju_llapndk_MainActivity_stringFromJNI(
        JNIEnv *env,
        jobject /* this */) {
...
extern "C"
JNIEXPORT jstring JNICALL
Java_cn_edu_nju_llapndk_MainActivity_getbaseband(JNIEnv *env, jobject instance, jshortArray data_,
...
extern "C"
JNIEXPORT jstring JNICALL
Java_cn_edu_nju_llapndk_MainActivity_initdownconvert(JNIEnv *env, jobject instance, jint samplerate,
                                                     jint numfreq, jdoubleArray wavfreqs_) {
    char result[255];
    int f, i, n;
    jdouble *wavfreqs = env->GetDoubleArrayElements(wavfreqs_, NULL);
...
extern "C"
JNIEXPORT jstring JNICALL
Java_cn_edu_nju_llapndk_MainActivity_removedc(JNIEnv *env, jobject instance, jdoubleArray data_,
    ...
    for (f = 0; f < c_numfreq; f++)//loop over all freqs
    {
        ...
        for (k = 0; k < 4; k++) {
            ...
            if (freqpower[sid] > thr * thr * cicnum) {
                if (max_val > maxvalue[sid] || ((max_val > minvalue[sid] + thrpeak) &&
                                                (maxvalue[sid] - minvalue[sid]) > thrpeak * 4 &&
                                                (max_id > 1) && ((cicnum - max_id) > 2))) {
                    maxvalue[sid] = max_val;
                }
                if (min_val < minvalue[sid] || ((min_val < maxvalue[sid] - thrpeak) &&
                                                (maxvalue[sid] - minvalue[sid]) > thrpeak * 4 &&
                                                (min_id > 1) && ((cicnum - min_id) > 2))) {
                    minvalue[sid] = min_val;
                }
                if (maxvalue[sid] - minvalue[sid] > thrpeak * 2)
                    dcvalue[sid] = (1 - dctrend) * dcvalue[sid] +
                                   (maxvalue[sid] + minvalue[sid]) / 2 * dctrend;
            } else {
...
extern "C"
JNIEXPORT jstring JNICALL
Java_cn_edu_nju_llapndk_MainActivity_getdistance(JNIEnv *env, jobject instance, jdoubleArray data_,
                                                 jdoubleArray outdata_, jdoubleArray dischange_,
                                                 jdoubleArray freqpow_) {
    char result[255];
    ...
    deltax = c_numfreq * ((cicnum - 1) * cicnum * (2 * cicnum - 1) / 6 -
                          (cicnum - 1) * cicnum * (cicnum - 1) / 4);
    for (f = 0; f < c_numfreq; f++)//loop over all freqs
    {
        l = f * cicstep;
        for (k = 0; k <= 2; k += 2) {
...
extern "C"
JNIEXPORT jstring JNICALL
Java_cn_edu_nju_llapndk_MainActivity_getidftdistance(JNIEnv *env, jobject instance,
...
extern "C"
JNIEXPORT jstring JNICALL
Java_cn_edu_nju_llapndk_MainActivity_calibrate(JNIEnv *env, jobject instance, jdoubleArray data_) {
    char result[255];
    double fftx[nfft];
    double ffty[nfft];
    ...
    if (fabs(diffdc) > 0.5) {
        ...
        sprintf(result, "calibrate max at %d target %f, curpos=%d,diff %f", maxid, targetid, curpos,
                diffdc);
    } else
